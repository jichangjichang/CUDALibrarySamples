...
std::vector<int> m_size = {8} ;
std::vector<int> n_size = {16} ;
std::vector<int> k_size = {32,};

for(int bert_index = 0; bert_index < 1; bert_index++){
...
//    __half hC[m * n] = {};
    ...
    for (int i = 0; i < m; i++) {
       std::printf("(%d, 0 ~ %d): ",  i, k-1);
       for (int j = 0; j < k; j++) {
                auto posA = i * lda + j;
                std::printf(" %.0f,",  static_cast<float>(hA[posA]));
       }
       std::printf("\n");
    }

    // PRINT compress A
    std::printf("A size = %d , comprssed size = %d\n", (int)A_size, (int) compressed_size);
    CHECK_CUDA( cudaMemcpy(hA, dA_compressed, compressed_size, cudaMemcpyDeviceToHost) )
    for (int i = 0; i < m; i++) {
       std::printf("(%d, 0 ~ %d): ",  i, k/2-1);
       for (int j = 0; j < k/2; j++) {
                auto posA = i * k/2 + j;
                std::printf(" %.0f,",  static_cast<float>(hA[posA]));
       }
    ...
    std::printf("print metadata\n");
    unsigned char* hMetadata =(unsigned char*) (hA + m*k/2);
    for (int i = 0; i < 4*m; i++) {
       std::printf("(%d, 0 ~ %d): ",  i, k/2-1);
       for (int j = 0; j < k/8; j++) {
                auto posA = i * k/8 + j;
                std::printf(" 0x%x,",  hMetadata[posA]);
       }
       std::printf("\n");
    }
    //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
